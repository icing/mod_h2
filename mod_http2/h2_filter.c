                                  ...
                                  apr_off_t readbytes) 
{
    ...
    if (!in->bb) {
        in->bb = apr_brigade_create(in->session->pool, f->c->bucket_alloc);
    }
    
    if (APR_BRIGADE_EMPTY(in->bb)) {
        ...
        status = ap_get_brigade(f->next, in->bb, AP_MODE_READBYTES,
                                block, readbytes);
    }
    
    switch (status) {
        case APR_SUCCESS:
            return consume_brigade(in, in->bb, block);
        case APR_EOF:
        case APR_EAGAIN:
            break;
        default:
